...
export default function ProjectHub() {
  const projects = [
    { id: '1', name: 'Stark Finance', status: 'Live', updated: '2h ago', type: 'Fintech Dashboard' },
    { id: '2', name: 'Lumina Portfolio', status: 'Draft', updated: '5h ago', type: 'Design System' },
    { id: '3', name: 'Vibe CRM', status: 'Building', updated: '12m ago', type: 'AI SaaS' },
    { id: '4', name: 'Ether Shop', status: 'Live', updated: '1d ago', type: 'E-commerce' }
  ];
    ...
    <>
      ...
      <main className="flex-1 flex flex-col w-full h-full text-foreground bg-background">
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 border-b border-border">
            ...
            {/* Create New Card */}
            <Link href="/studio/new" className="flex flex-col items-center justify-center p-12 border-r border-b lg:border-b-0 border-border bg-background cursor-pointer hover:bg-panel hover:border-accent transition-all group">
                ...
                <div key={project.id} className="flex flex-col border-r border-b lg:border-b-0 border-border bg-panel p-8 h-80 relative group hover:bg-background transition-colors">
                    <div className="flex justify-between items-start mb-6">
                        <div className="space-y-2">
                            <Link href={`/studio/${project.id}`} className="text-2xl font-bold tracking-tight hover:text-accent transition-colors leading-none block">{project.name}</Link>
                            <p className="text-[10px] font-mono uppercase tracking-widest opacity-40">{project.type}</p>
                        </div>
                        <div className="flex items-center space-x-2 border border-border px-2 py-1">
                            <div className="w-1.5 h-1.5" style={{ background: project.status === 'Draft' ? '#444' : 'var(--accent)' }}></div>
                    ...
                    <div className="mt-auto flex justify-between items-center z-10 relative pt-4">
                        <span className="text-[10px] font-mono opacity-40 uppercase tracking-widest">UPD_ {project.updated}</span>
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex space-x-3">
                            <Link href={`/insights/${project.id}`} className="hover:text-accent transition-colors"><ExternalLink className="w-4 h-4" /></Link>
                            <Link href={`/deploy/${project.id}`} className="hover:text-accent transition-colors"><Settings className="w-4 h-4" /></Link>
                        </div>
                    </div>
                </div>
            ))}
